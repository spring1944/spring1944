...
function widget:Initialize()
	for unitDefID=1, #UnitDefs do
		...
		local penetration, dropoff, radiusrange, penetrationheat
		for i=1,#unitDef.weapons do
			...
			if (customParams.damagetype ~= "grenade") then
				if (customParams.damagetype == "shapedcharge") then 
					...
					penetrationheat = tonumber(armor_penetration)
					radiusrange = range
				else
		...
		if penetration or penetrationheat then
			infos[unitDefID] = {penetration, dropoff, radiusrange, penetrationheat}
		end
	end
	...
	if selectedUnit then
		...
		if cmd == CMD_ATTACK then
			...
			if infos[unitDefID] then
				...
				if radiusrange and radiusrange > dist then 
